...
function renderDashboard() {
    ...
    const totals = matches.reduce((acc, match) => {
        ...
        acc.possession += Number(match.possession || 0);
        return acc;
    }, { wins: 0, draws: 0, passAcc: 0, possession: 0 });
    ...
    const avgPossession = totalMatches > 0 ? totals.possession / totalMatches : 0;
...
function renderPositiveSummary({ unbeatenStreak, ppg, avgPossession, avgPassAcc }) {
    ...
    const items = [
        ...
        { label: "Posesie medie", value: `${avgPossession.toFixed(1)}%` },
